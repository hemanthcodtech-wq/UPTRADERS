...
async function sendOTPEmail(email, otp, name) {
  await transporter.sendMail({
    from: `"UP Traders" <${process.env.EMAIL_USER}>`,
    to: email,
    subject: 'Your OTP for UP Traders Signup',
      html: `
        <div style="font-family:Arial,sans-serif;max-width:480px;margin:auto;padding:24px;border:2px solid #106935;border-radius:12px;background-color:#ffffff;">
          <div style="text-align:center;margin-bottom:20px;">
            <h2 style="color:#106935;margin:0;font-size:24px;">UP Traders</h2>
          ...
          <div style="font-size:36px;font-weight:bold;color:#106935;letter-spacing:8px;text-align:center;padding:16px;background:#f0fdf4;border:1px dashed #106935;border-radius:8px;margin:20px 0;">
            ${otp}
          </div>
...
async function sendSMSOTP(phone, otp, name) {
  ...
  try {
    await twilioClient.messages.create({
      body: `Your UP Traders OTP is ${otp}. Valid for 10 minutes.`,
...
router.post('/forgot-password', async (req, res) => {
  ...
  try {
    ...
    await transporter.sendMail({
      from: `"UP Traders" <${process.env.EMAIL_USER}>`,
      to: email,
      subject: 'Reset Your UP Traders Password',
      html: `
        <div style="font-family:Arial,sans-serif;max-width:480px;margin:auto;padding:24px;border:2px solid #106935;border-radius:12px;background-color:#ffffff;">
          <div style="text-align:center;margin-bottom:20px;">
            <h2 style="color:#106935;margin:0;font-size:24px;">UP Traders</h2>
