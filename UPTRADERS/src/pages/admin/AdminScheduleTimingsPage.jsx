  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-brand-green/10 shadow-sm overflow-hidden">
        <div className="border-b border-brand-green/10 px-6 py-4 flex items-center gap-3 bg-[#FDF8F0]">
            ...
            <div className="flex flex-wrap gap-2">
              {PRESETS.filter(p => !slots.find(s => s.label === p)).map(preset => (
                <button
                  ...
                  className="flex items-center gap-1.5 text-xs font-bold text-gray-900/70 bg-[#FDF8F0] border border-brand-green/20 hover:border-brand-green/50 hover:bg-brand-green/5 px-3 py-1.5 rounded-full transition-colors"
                >
      ...
      <div className="bg-white rounded-2xl border border-brand-green/10 shadow-sm overflow-hidden">
        ...
        {slots.length === 0 ? (
          ...
          <div className="divide-y divide-brand-green/5">
            {slots.map((slot, idx) => (
              <div key={idx}
                className={`flex items-center gap-4 px-6 py-4 transition-colors ${slot.enabled !== false ? 'bg-white hover:bg-[#FDF8F0]/50' : 'bg-gray-50/80'}`}
              >
