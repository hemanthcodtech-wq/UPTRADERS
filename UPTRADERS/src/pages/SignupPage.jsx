        ...
        <React.Fragment key={i}>
          <div className="flex flex-col items-center gap-1 flex-shrink-0">
            <div className={`${mobile ? 'w-8 h-8' : 'w-7 h-7'} rounded-full flex items-center justify-center text-xs font-bold transition-all duration-300 ${
              ...
              i === activeIdx ? 'bg-[#106935] text-white ring-4 ring-[#106935]/20' :
            ...
            <span className={`${mobile ? 'text-[10px]' : 'text-[9px]'} font-semibold tracking-wide uppercase ${
              i === activeIdx ? 'text-[#106935]' : i < activeIdx ? 'text-green-600' : 'text-gray-300'
            }`}>{label}</span>
          </div>
          {i < steps.length - 1 && (
  ...
  return (
    <button
      ...
      className={`group relative w-full flex flex-col items-center gap-3 ${mobile ? 'p-5' : 'p-6'} rounded-2xl border-2 transition-all duration-300 cursor-pointer active:scale-95 ${
        selected
          ? 'border-[#106935] bg-gradient-to-br from-red-50 to-rose-50 shadow-lg shadow-red-100 ring-4 ring-[#106935]/10'
          : 'border-gray-200 bg-white hover:border-[#106935]/40 hover:shadow-md'
      }`}
    >
      <div className={`${mobile ? 'w-14 h-14' : 'w-16 h-16'} rounded-2xl flex items-center justify-center transition-all duration-300 ${
        selected ? 'bg-[#106935] shadow-lg shadow-[#106935]/30' : 'bg-gray-100 group-hover:bg-[#106935]/10'
      }`}>
        {isShopkeeper
          ? <Store className={`${mobile ? 'w-7 h-7' : 'w-8 h-8'} transition-colors ${selected ? 'text-white' : 'text-gray-400 group-hover:text-[#106935]'}`} />
          : <UserCircle2 className={`${mobile ? 'w-7 h-7' : 'w-8 h-8'} transition-colors ${selected ? 'text-white' : 'text-gray-400 group-hover:text-[#106935]'}`} />
        }
      </div>
      <div className="text-center">
        <p className={`${mobile ? 'text-base' : 'text-lg'} font-bold transition-colors ${selected ? 'text-[#106935]' : 'text-gray-900'}`}>
      ...
      {selected && (
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
          className="absolute top-2.5 right-2.5 w-5 h-5 rounded-full bg-[#106935] flex items-center justify-center">
...
export function SignupPage() {
  ...
  const inputCls = "w-full bg-white border border-gray-200 shadow-sm rounded-xl px-4 pl-11 py-3 text-sm text-gray-900 placeholder:text-gray-400 focus:outline-none focus:border-[#106935] focus:ring-4 focus:ring-[#106935]/10 hover:border-gray-300 transition-all duration-200";

  const RoleBadge = ({ onClick }) => selectedRole ? (
    <button type="button" onClick={onClick || (() => setStep('role'))}
      className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-[#106935]/10 text-[#106935] text-xs font-semibold border border-[#106935]/20 hover:bg-[#106935]/20 transition-colors mb-2">
      {selectedRole === 'shopkeeper' ? <Store className="w-3 h-3" /> : <UserCircle2 className="w-3 h-3" />}
      {selectedRole === 'shopkeeper' ? 'Shopkeeper' : 'Customer'}
      <span className="text-[#106935]/50 ml-0.5">· change</span>
    </button>
  ) : null;
      ...
      <motion.div key={step} variants={slideVariants} initial="enter" animate="center" exit="exit"
        ...
        {step === 'role' && (
          <div className="space-y-5">
            ...
            <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}
              ...
              className="w-full bg-gradient-to-r from-[#106935] to-[#15803d] text-white font-bold py-3.5 rounded-xl text-sm shadow-md hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 flex items-center justify-center gap-2">
            ...
            <p className="text-center text-sm text-gray-500">
              ...
              <Link to="/login" className="font-bold text-gray-900 hover:text-[#106935] transition-colors">Sign In</Link>
            </p>
          </div>
        )}
            ...
            <form onSubmit={handleSignup} className="space-y-3">
              <div>
                ...
                <div className="relative group">
                  <User className="w-4 h-4 text-gray-400 absolute left-4 top-1/2 -translate-y-1/2 group-focus-within:text-[#106935] transition-colors" />
              ...
              <div>
                ...
                <div className="relative group">
                  <Mail className="w-4 h-4 text-gray-400 absolute left-4 top-1/2 -translate-y-1/2 group-focus-within:text-[#106935] transition-colors" />
              ...
              <div>
                ...
                <div className="relative group">
                  <Lock className="w-4 h-4 text-gray-400 absolute left-4 top-1/2 -translate-y-1/2 group-focus-within:text-[#106935] transition-colors" />
                  ...
                  <button type="button" onClick={() => setShowPass(p => !p)}
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#106935] transition-colors">
              ...
              <label className="flex items-start gap-2.5 cursor-pointer pt-1">
                <input type="checkbox" checked={consentAccepted} onChange={e => setConsentAccepted(e.target.checked)} className="mt-0.5 w-4 h-4 accent-[#106935] rounded shrink-0" />
                <span className="text-xs text-gray-500 leading-relaxed">
                  I confirm I am 13+ and agree to the{' '}
                  <Link to="/terms-of-service" target="_blank" className="font-bold text-gray-900 underline hover:text-[#106935]">Terms & Conditions</Link> and{' '}
                  <Link to="/privacy-policy" target="_blank" className="font-bold text-gray-900 underline hover:text-[#106935]">Privacy Policy</Link>.
                </span>
              </label>
              ...
              <motion.button whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.98 }}
                ...
                className="w-full bg-gradient-to-r from-[#106935] to-[#15803d] text-white font-bold py-3.5 rounded-xl text-sm shadow-md hover:shadow-xl hover:-translate-y-0.5 transition-all duration-300 disabled:opacity-60 flex items-center justify-center gap-2">
        ...
        {step === 'otp' && (
          <div className="space-y-6">
            <div className="text-center">
              <RoleBadge />
              <div className="w-16 h-16 rounded-2xl bg-[#106935]/10 flex items-center justify-center mx-auto mb-4">
                <Mail className="w-8 h-8 text-[#106935]" />
              </div>
      ...
      <motion.div key={step} variants={slideVariants} initial="enter" animate="center" exit="exit"
        ...
        {step === 'role' && (
          <div className="space-y-4">
            ...
            <motion.button whileTap={{ scale: 0.97 }}
              type="button" onClick={handleRoleContinue}
              className="w-full bg-gradient-to-r from-[#106935] to-[#15803d] text-white font-bold py-4 rounded-2xl text-base shadow-lg shadow-red-200 flex items-center justify-center gap-2 active:brightness-95 transition-all">
            ...
            <p className="text-center text-sm text-gray-500 pb-2">
              Have an account?{' '}
              <Link to="/login" className="font-bold text-[#106935]">Sign In</Link>
            </p>
          </div>
        )}
            ...
            <form onSubmit={handleSignup} className="space-y-4">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input name="name" value={form.name} onChange={handleChange} required
                    ...
                    className="w-full bg-white border border-gray-200 rounded-2xl px-4 pl-11 py-3.5 text-[15px] text-gray-900 placeholder:text-gray-400 focus:outline-none focus:border-[#106935] focus:ring-4 focus:ring-[#106935]/10 shadow-sm transition-all" />
                </div>
              </div>
                  ...
                  <input name="password" type={showPass ? 'text' : 'password'} value={form.password}
                    ...
                    className={`w-full bg-white border rounded-2xl px-4 pl-11 pr-12 py-3.5 text-[15px] text-gray-900 placeholder:text-gray-400 focus:outline-none focus:ring-4 shadow-sm transition-all ${
                      passwordTouched && !passwordValid
                        ...
                        : 'border-gray-200 focus:border-[#106935] focus:ring-[#106935]/10'
                    }`} />
                  <button type="button" onClick={() => setShowPass(p => !p)}
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400 hover:text-[#106935] p-1 transition-colors">
              ...
              <label className="flex items-start gap-3 cursor-pointer bg-gray-50 rounded-2xl p-3.5 border border-gray-200 active:bg-gray-100 transition-colors">
                <input type="checkbox" checked={consentAccepted} onChange={e => setConsentAccepted(e.target.checked)}
                  className="mt-0.5 w-5 h-5 accent-[#106935] rounded shrink-0" />
                <span className="text-xs text-gray-500 leading-relaxed">
              ...
              <motion.button whileTap={{ scale: 0.97 }}
                type="submit" disabled={loading}
                className="w-full bg-gradient-to-r from-[#106935] to-[#15803d] text-white font-bold py-4 rounded-2xl text-base shadow-lg shadow-red-200 disabled:opacity-60 flex items-center justify-center gap-2 active:brightness-95 transition-all">
        ...
        {step === 'otp' && (
          <div className="space-y-6">
            ...
            <div className="text-center">
              <RoleBadge />
              <div className="w-20 h-20 rounded-3xl bg-gradient-to-br from-[#106935]/10 to-rose-100 flex items-center justify-center mx-auto mb-5 shadow-inner">
                <Mail className="w-10 h-10 text-[#106935]" />
              </div>
              <h2 className="text-2xl font-extrabold text-gray-900 tracking-tight">Check your email</h2>
  ...
  return (
    <>
      ...
      <div className="lg:hidden min-h-screen w-full font-sans flex flex-col" style={{ background: 'linear-gradient(170deg, #fff5f5 0%, #fff 35%)' }}>
        ...
        <div className="relative bg-gradient-to-br from-[#106935] to-emerald-900 px-6 pt-10 pb-16 overflow-hidden">
          {/* Decorative circles */}
          ...
          <div className="relative z-10 inline-block bg-white rounded-xl py-1.5 px-3 shadow-[0_0_25px_rgba(255,255,255,0.6)] border border-white">
            <img src={logoImg} alt="UP Traders Logo" className="h-7 w-auto object-contain" />
          </div>
